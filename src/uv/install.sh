#!/usr/bin/env bash

set -e

# Clean up
rm -rf /var/lib/apt/lists/*

UV_VERSION=${VERSION:-"latest"}

if [ "$(id -u)" -ne 0 ]; then
    echo -e 'Script must be run as root. Use sudo, su, or add "USER root" to your Dockerfile before running this script.'
    exit 1
fi

apt_get_update() {
    echo "Running apt-get update..."
    apt-get update -y
}

# Checks if packages are installed and installs them if not
check_packages() {
    if ! dpkg -s "$@" > /dev/null 2>&1; then
        if [ "$(find /var/lib/apt/lists/* | wc -l)" = "0" ]; then
            apt_get_update
        fi
        apt-get -y install --no-install-recommends "$@"
    fi
}

export DEBIAN_FRONTEND=noninteractive

find_version_from_git_tags() {
    local variable_name=$1
    local requested_version=${!variable_name}
    if [ "${requested_version}" = "none" ]; then return; fi
    local repository=$2
    local prefix=${3:-"tags/"}
    local separator=${4:-"."}
    local last_part_optional=${5:-"false"}
    local version_list
    if [ "$(echo "${requested_version}" | grep -o "." | wc -l)" != "2" ]; then
        local escaped_separator=${separator//./\\.}
        local last_part
        if [ "${last_part_optional}" = "true" ]; then
            last_part="(${escaped_separator}[0-9]+)?"
        else
            last_part="${escaped_separator}[0-9]+"
        fi
        local regex="${prefix}\\K[0-9]+${escaped_separator}[0-9]+${last_part}$"
        version_list="$(git ls-remote --tags ${repository} | grep -oP "${regex}" | tr -d ' ' | tr "${separator}" "." | sort -rV)"
        if [ "${requested_version}" = "latest" ] || [ "${requested_version}" = "current" ] || [ "${requested_version}" = "lts" ]; then
            declare -g ${variable_name}="$(echo "${version_list}" | head -n 1)"
        else
            set +e
            declare -g ${variable_name}="$(echo "${version_list}" | grep -E -m 1 "^${requested_version//./\\.}([\\.\\s]|$)")"
            set -e
        fi
    fi
    if [ -z "${!variable_name}" ]; then
        if [ -z "${version_list}" ]; then
            local escaped_separator=${separator//./\\.}
            local last_part
            if [ "${last_part_optional}" = "true" ]; then
                last_part="(${escaped_separator}[0-9]+)?"
            else
                last_part="${escaped_separator}[0-9]+"
            fi
            local regex="${prefix}\\K[0-9]+${escaped_separator}[0-9]+${last_part}$"
            version_list="$(git ls-remote --tags ${repository} | grep -oP "${regex}" | tr -d ' ' | tr "${separator}" "." | sort -rV)"
        fi
        echo -e "Invalid ${variable_name} value: ${requested_version}\nValid values:\n${version_list}" >&2
        exit 1
    fi
    echo "${variable_name}=${!variable_name}"
}

# Install dependencies
check_packages curl ca-certificates git tar

architecture="$(uname -m)"
case "${architecture}" in
    x86_64) architecture="x86_64" ;;
    aarch64 | armv8* | arm64) architecture="aarch64" ;;
    *) echo "(!) Architecture ${architecture} unsupported" >&2; exit 1 ;;
esac

TMP_DIR="$(mktemp -d -t uv-XXXXXX)"
chmod 700 "${TMP_DIR}"

install_uv_archive() {
    local target archive_url archive_path extracted_dir
    for target in "$@"; do
        archive_url="https://github.com/astral-sh/uv/releases/download/${UV_VERSION}/${target}.tar.gz"
        archive_path="${TMP_DIR}/${target}.tar.gz"
        echo "    Attempting download for ${target}"
        if curl -fsSL -o "${archive_path}" "${archive_url}"; then
            if tar -xzf "${archive_path}" -C "${TMP_DIR}" "${target}/uv" "${target}/uvx"; then
                extracted_dir="${TMP_DIR}/${target}"
                cp "${extracted_dir}/uv" /usr/local/bin/uv
                cp "${extracted_dir}/uvx" /usr/local/bin/uvx
                chmod 0755 /usr/local/bin/uv /usr/local/bin/uvx
                return 0
            else
                echo "    Failed to extract ${target}; trying next option" >&2
            fi
        else
            echo "    Failed to download ${archive_url}; trying next option" >&2
        fi
    done
    return 1
}

setup_uv() {
    tee /usr/local/share/uv-init.sh << 'EOF'
#!/bin/sh
set -e

# Resolve USERNAME chosen during install from this script's owner, fallback to common users/root
OWNER="$(stat -c '%U' /usr/local/share/uv-init.sh 2>/dev/null || true)"
if [ -n "$OWNER" ] && [ "$OWNER" != "root" ]; then
    USERNAME="$OWNER"
elif [ -z "$USERNAME" ] || ! id -u "$USERNAME" >/dev/null 2>&1; then
    for u in vscode node codespace "$(awk -F: '$3==1000{print $1}' /etc/passwd)"; do
        if id -u "$u" >/dev/null 2>&1; then
            USERNAME="$u"
            break
        fi
    done
    [ -z "$USERNAME" ] && USERNAME="root"
fi

mkdir -p /var/lib/uv/cache \
    && chown -R "$USERNAME":"$USERNAME" /var/lib/uv/cache \
    && chmod 0750 /var/lib/uv/cache

set +e

# Execute whatever commands were passed in (if any). This allows us
# to set this script to ENTRYPOINT while still executing the default CMD.
exec "$@"
EOF
    chmod +x /usr/local/share/uv-init.sh \
        && chown ${USERNAME}:root /usr/local/share/uv-init.sh
}

echo "(*) Installing uv..."
find_version_from_git_tags UV_VERSION https://github.com/astral-sh/uv "tags/"

UV_VERSION="${UV_VERSION#v}"

echo "    Using version ${UV_VERSION}"

uv_targets=()
case "${architecture}" in
    x86_64)
        uv_targets+=("uv-x86_64-unknown-linux-musl")
        uv_targets+=("uv-x86_64-unknown-linux-gnu")
        ;;
    aarch64)
        uv_targets+=("uv-aarch64-unknown-linux-musl")
        uv_targets+=("uv-aarch64-unknown-linux-gnu")
        ;;
    *)
        echo "(!) Architecture ${architecture} unsupported" >&2
        exit 1
        ;;
esac

if ! install_uv_archive "${uv_targets[@]}"; then
    echo "    Unable to install uv for architecture ${architecture}" >&2
    exit 1
fi

setup_uv

rm -rf /var/lib/apt/lists/* "${TMP_DIR}"

echo "Done!"
