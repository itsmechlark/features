...
PG_ARCHIVE_VERSION_CODENAMES="bookworm bullseye buster sid bionic focal jammy kinetic noble trixie"
...
if [ ! -f "$PGDATA/PG_VERSION" ]; then
    ...
    chown -R postgres:postgres $PGDATA \
        && chmod 0750 $PGDATA \
        && sudo -H -u postgres sh -c "/usr/lib/postgresql/${version_major}/bin/initdb -D $PGDATA --auth trust --auth-local trust --auth-host trust" \
